...
import random
import time
tantos = ["A", "2", "3", "4", "5", "6", "7", "S", "C", "R",] #Card Suits "#Cartas_S
palos=["monedas", "copas", "espadas", "bastos",] #Faces_Ace
cartas_d = [] #cards_deck
num_car = 40
...
class Carta():
    def __init__(self,palo,num): #rank=palo suit=num
        self.palo = palo
        self.num   = num
    def mostrar_carta(self): #showcard
        print('{} de {}'.format(self.palo, self.num))

class deck_Baraja(): #DeckOfCards
    def __init__(self):
        print("Se mostrará en texto la baraja ...")
        cartas_n = [Carta(palo,num) for palo in range(1,1) for num in tantos] #regularcards
        cara_carta = [Carta(palo,num) for palo in palos for num in tantos] #face_ace_card
        self.baraja_cartas = cartas_n+cara_carta  #deck_of_cards
